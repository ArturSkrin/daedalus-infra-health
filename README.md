...
Значення `drill`: `users`, `forecast`, `trust`, `day`, `verdict`. Готові PNG усіх десяти зрізів і пʼяти drill-in лежать у [design/](design/).
...
Правила пʼяти показників живуть в одному місці, [backend/engine.py](backend/engine.py). Їх перевіряють 20 тестів: рішення і стани для кожного з десяти зрізів, плюс рядок причини на екрані.

```bash
docker compose run --rm -v "$PWD:/src" -w /src -e PYTHONPATH=/src backend python -m pytest -q
```
...
- **engine.py** вирішує. Приймає пʼять блоків у формі Triage API і повертає стани чотирьох показників, рішення і правило, яке спрацювало.
- **presentation.py** перекладає рішення на людську мову: слово, рядок причини, підписи плиток, заповнення кілець, вміст drill-in, чергу. Порогів тут немає.
- **frontend** нічого не рахує і не знає жодного порога. Сирі величини зʼявляються лише в drill-in.
- **live.py** збирає ті самі пʼять блоків зі справжнього Triage. Живий режим вмикається змінними `TRIAGE_BASE_URL`, `TRIAGE_TOKEN`, `TRIAGE_TENANT`, після чого у списку зрізів зʼявляється пункт Live data. Без доступу до core організаторів цей шлях не перевірено на живих даних.
...
| [backend/](backend/) | FastAPI: engine, presentation, live, експорт статичних моделей |
| [frontend/](frontend/) | React, Tailwind, Vite |
| [tests/](tests/) | pytest на всі зрізи |
...
| Існує стан «все гаразд» | ALL CLEAR у зрізах S01, S02, S06, S10 | design/phone_s01_calm.png |
...
| Мінімум 8 зрізів із вхідними значеннями і рішенням | 10 зрізів, 20 тестів | scenarios.md, tests/ |
...
| Виявлені прогалини в каталозі | 8, три винесено на номінацію | design_rationale.md |
| Реалізація на живих даних | адаптер готовий, на живому core не перевірено | backend/live.py |
